...
def simulate_stochastic_generic(pathway_name, n_simulations=2000, total_years=40,
                                 sigma_annual=DEFAULT_SIGMA_ANNUAL, seed=None):
    """
    Generic stochastic wrapper for pathways with no discrete structural
    event modelled (industry_data, industry_quant): log-normal noise
    around the 'central' deterministic trajectory.
    ...
    shocks = _lognormal_multiplicative_shocks(n_simulations, total_years, sigma_annual, rng)
    return base[None, :] * shocks
...
def simulate_stochastic_academic(n_simulations=2000, total_years=40,
    ...
    converts to a permanent position, on top of smooth log-normal noise.
